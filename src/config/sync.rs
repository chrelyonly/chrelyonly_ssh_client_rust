...
fn load_account(username: &str) -> Result<Option<SyncAccount>> {
    let primary = account_path(username);
    let legacy = legacy_account_path(username);
            ...
            match fs::read_to_string(&legacy) {
                ...
                Err(legacy_error) => {
                    return Err(legacy_error)
                        .with_context(|| format!("读取同步账号失败 {:?}", legacy));
                }
            }
        }
        Err(error) => {
            return Err(error).with_context(|| format!("读取同步账号失败 {:?}", primary));
        }
    };
...
fn legacy_account_path(username: &str) -> PathBuf {
    legacy_config_dir()
        .join(SYNC_DIR_NAME)
        .join(format!("{}.json", slugify(username)))
}
